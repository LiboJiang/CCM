ES.plink <- function(SNP,fil="1"){
  
  nsnp <- dim(SNP)[1]
  nm <- dim(SNP)[2]
  SNPname <- rownames(SNP)
  SNP.1 <- t(SNP)
  rownames(SNP.1) <- NULL
  ...
  MAP <- as.data.frame(cbind(INDEX1,SNPname,INDEX2,INDEX2))
  file.name <- paste(fil,".map",sep="")
  write.table(MAP,file=file.name,row.names = F,col.names = F,quote=F)
  
  INDEX1 <- rep(-9,nm)
  INDEX2 <- rep(1,nm)
  INDEX3 <- 1:nm
  
  FAM <- cbind(INDEX3,INDEX3,INDEX3,INDEX3,INDEX2,INDEX1)
  NSNP <- c()
  for(i in 1:nsnp){
    A <- rep(i,2)
    NSNP <-c(NSNP,A)
  }
  SNP.2 <- snp.tr(snp.l=SNP.1)
  ped <- as.data.frame(cbind(FAM,SNP.2[,NSNP]))
  
  file.name1 <- paste(fil,".ped",sep="")
  write.table(ped,file.name1,row.names = F,col.names = F,quote=F)
}
...
snp.tr <- function(snp.l){
  
  nn <- dim(snp.l)[2]
  
  for(i in 1:nn){
    res <- names(table(snp.l[,i]))
    snp.l[which(snp.l[,i]==res[1]),i] <- 1
    snp.l[which(snp.l[,i]==res[2]),i] <- 2
  }
  return(snp.l)
}
